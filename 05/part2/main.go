	...
	for _, line := range lines {
		...
		if line.y1 == line.y2 {
			...
			for {
				...
				// increment x/y if x1/y1 is lower than x2/y2
				if isX1Less && line.x1 <= line.x2 {
					line.x1++
				}
				if isY1less && line.y1 <= line.y2 {
					line.y1++
				}

				// decrement x/y if x1/y1 is higher than x2/y2
				if !isX1Less && line.x1 >= line.x2 {
					line.x1--
				}
				if !isY1less && line.y1 >= line.y2 {
